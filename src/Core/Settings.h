...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(Bool, parallel_replicas_allow_in_with_subquery, true, "Effective only for READ_TASKS mode. If true, subquery for IN will be executed on every follower replica.", 0) \
    M(Float, parallel_replicas_single_task_marks_count_multiplier, 2, "Effective only for READ_TASKS mode. A multiplier which will be added during calculation for minimal number of marks to retrieve from coordinator. This will be applied only for remote replicas.", 0) \
    M(Bool, parallel_replicas_for_non_replicated_merge_tree, false, "Effective only for READ_TASKS mode. If true, ClickHouse will use parallel replicas algorithm also for non-replicated MergeTree tables", 0) \
    M(UInt64, parallel_replicas_min_number_of_rows_per_replica, 0, "Effective only for READ_TASKS mode. Limit the number of replicas used in a query to (estimated rows to read / min_number_of_rows_per_replica). The max is still limited by 'max_parallel_replicas'", 0) \
    M(Bool, parallel_replicas_prefer_local_join, true, "Effective only for READ_TASKS mode. If true, and JOIN can be executed with parallel replicas algorithm, and all storages of right JOIN part are *MergeTree, local JOIN will be used instead of GLOBAL JOIN.", 0) \
    M(UInt64, parallel_replicas_mark_segment_size, 128, "Effective only for READ_TASKS mode. Parts virtually divided into segments to be distributed between replicas for parallel reading. This setting controls the size of these segments. Not recommended to change until you're absolutely sure in what you're doing", 0) \
    \
